...
def migrate(cr, version):
    ...
    cr.execute("""
                   ...
                   (SELECT res_id FROM ir_model_data WHERE model = 'ir.ui.view' AND module in ('univision_sale'))
           """)
    ...
    cr.execute("""
                   DELETE FROM ir_model_data WHERE model = 'ir.actions.act_window' AND module IN ('univision_sale')
           """)

    # TO UNINSTALL CUSTOM MODULE
    cr.execute("""
                   UPDATE ir_module_module SET state = 'to remove' WHERE name = 'univision_sale'
           """)
